...
import type { LandlordNotification } from "@/lib/landlord-types";
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const data = (await notifsRes.json()) as {
          ...
          {loading ? (
            <div className="space-y-3">
              {Array.from({ length: 4 }).map((_, i) => (
                <div
                  key={i}
                  className="h-20 animate-pulse rounded-xl border border-warm-gray/10 bg-surface"
                />
              ))}
            </div>
          ) : error ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-muted/60 text-xs mt-1">
                Notifications will appear when tenants match, message, or request tours.
              </p>
            </div>
          ) : (
            notifications.map((notification, i) => {
              ...
              return (
                ...
                >
                  <Link
                    href={notification.listingId ? `/landlord/dashboard/${notification.listingId}` : "#"}
                          ...
                          <div className="flex-1">
                            <p
                              className={`text-sm font-medium ${
                                !notification.unread ? "text-foreground/70" : "text-foreground"
                              }`}
                            >
                              {notification.title}
                            </p>
